...
FCGUlt::FCGUlt(socd::SocdType socd_type) {
...
void FCGUlt::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
    outputs.b = inputs.b;
    outputs.x = inputs.x;
    outputs.y = inputs.y;
...
void FCGUlt::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
...
// Custom SOCD handling for left/right when down is pressed
if (inputs.down) {
    if (inputs.left && inputs.right) {
        // Set horizontal to neutral
        outputs.leftStickX = ANALOG_STICK_NEUTRAL;
    }
} else {
    // Apply normal SOCD (2IP) for left/right when down is not pressed
    if (inputs.left && inputs.right) {
        outputs.leftStickX = inputs.right ? ANALOG_STICK_MAX : ANALOG_STICK_MIN;
    }
}
    /* ------------------ Shield Modifiers ------------------ */
    ...
    if (inputs.mod_x) {
        ...
        if (directions.horizontal) {                                // Fastest walk = 80.
            outputs.leftStickX = 128 + (directions.x * 71);         // Max value for walk out-of-dash and teeter = 71 (GCC = 53).
        }
        // MX + Vertical (1/3 DI). MX + Diagonal (Medium walk).
        if (directions.vertical) {
            outputs.leftStickX = 128 + (directions.x * 50);         // Max value for Tink/Yink passive shield = 50 (GCC = 40).
